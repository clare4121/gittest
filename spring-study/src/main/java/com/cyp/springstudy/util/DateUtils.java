...
public class DateUtils {
    ...
    private static final SimpleDateFormat sdf1 = new SimpleDateFormat( "yyyyMM" );
    /**
     * 时间字符串转化成为时间戳
     * @param str
     * @return
     ...
     * @return
     */
    public static String getOneYearBefore(String currentDate) {
        String onYearBefore = "";
        try {
            calendar.setTime(sdf1.parse(currentDate));
            calendar.add(Calendar.YEAR, -1);
            onYearBefore = sdf1.format(calendar.getTime());
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return onYearBefore;
    ...
    public static String getOneMonthBefore() {
        ...
        String oneMonthBefore = sdf1.format(calendar.getTime());
     ...
     * @return 返回格式-YYYYMM
     */
    public static String getOneMonthBefore(String currentDate) {
        String onMonthBefore = "";
        try {
            calendar.setTime(sdf1.parse(currentDate));
            calendar.add(Calendar.MONTH, -1);
            onMonthBefore = sdf1.format(calendar.getTime());
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return onMonthBefore;
    }
     ...
     * @return []
     */
    public static List<String> getNMonthBefore(String currentDate, Integer num) {
        List<String> monthsArr = new ArrayList<>();
        try {
            for (Integer i = 0; i < num; i++) {
                calendar.setTime(sdf1.parse(currentDate));
                calendar.add(Calendar.MONTH, -i);
                monthsArr.add(sdf1.format(calendar.getTime()));
            }
            Collections.reverse(monthsArr);
        } catch (ParseException e) {
            e.printStackTrace();
        }
        return monthsArr;
